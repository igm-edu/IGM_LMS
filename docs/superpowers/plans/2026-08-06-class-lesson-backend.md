...
**주의: 이 태스크도 리뷰 후 수정으로 테스트 4개가 더 붙어 177개로 끝났다.
아래 예상 개수는 그것을 반영한 값이다.**
...
Run: `npm test`
Expected: 191 tests pass (177 + 신규 14)

**주의: 이 태스크도 리뷰 후 수정으로 테스트 3개가 더 붙어 194개로 끝났다.**
...
Run: `npm test`
Expected: 200 tests pass (194 + 신규 6)
...
원래대로 되돌리고 다시 `npm test`로 200개 통과를 확인한다. 실패하지 않았다면 테스트가 제 역할을 못 하는 것이므로 멈추고 보고할 것.
...
- `npm test`가 200개 테스트를 통과하고 출력이 깨끗하다.
